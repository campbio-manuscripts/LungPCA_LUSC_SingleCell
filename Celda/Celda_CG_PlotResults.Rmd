...
if (!is.null(cellAnnotFinal)) {
  if (!all(cellAnnotFinal %in% colnames(colData(altExp(sce))))) {
    s <-
      setdiff(cellAnnotFinal, colnames(colData(altExp(sce))))
    stop(
...
### Clusters
```{r celda_clusters}
ggplotly(plotDimReduceCluster(sce, reducedDimName = reducedDimName, labelClusters = FALSE, size = 0.25), showLegend = TRUE)
```

### Sample Labels
```{r celda_samples}
ggplotly(plotSCEDimReduceColData(
  altExp(sce),
  reducedDimName = reducedDimName,
  colorBy = "celda_sample_label", # PCGA02_CellID_Template
  labelClusters = FALSE,
  dotSize = 0.25,
  legendSize = 5),
  tooltip = c("color"))
...
```{r celda_cellAnnot, results = "asis", fig.width=8, fig.height=6}
cat(sprintf(tab4, "Subject"))
nb.cols <- length(unique(sce$Subject))
...
cat(sprintf(tab4, "Sample"))
nb.cols <- length(unique(sce$Sample))
mycolors <- colorRampPalette(brewer.pal(12, "Paired"))(nb.cols)
ggplotly(plotSCEDimReduceColData(
  altExp(sce),
  colorBy = "Sample",
...
cat(sprintf(tab4, "Histology"))
hist_cols <- readRDS("/restricted/projectnb/pcga/PCGA2_BronchialPML_SingleCell_Biopsies+Brushes/Analysis/Molecular_Subtyping/histology_cols.rds")
ggplotly(plotSCEDimReduceColData(
        altExp(sce),
        colorBy = "Histology",
        colorScale = hist_cols,
        reducedDim = reducedDimName,
        labelClusters = FALSE,
        dotSize = 0.25,
        legendSize = 5),
...
cat(sprintf(tab4, "Smoking Status"))
smoking_cols <- readRDS("/restricted/projectnb/pcga/PCGA2_BronchialPML_SingleCell_Biopsies+Brushes/Analysis/Molecular_Subtyping/smoking_cols.rds")
ggplotly(plotSCEDimReduceColData(
        altExp(sce),
        colorBy = "Smoking_Status",
        colorScale = smoking_cols,
        reducedDim = reducedDimName,
        labelClusters = FALSE,
        dotSize = 0.25,
        legendSize = 5),
...
cat(sprintf(tab4, "Batch"))
nb.cols <- length(unique(sce$Batch))
...
cat(sprintf(tab4, "Flow"))
nb.cols <- length(unique(sce$Flow))
...
Use the tabs to select modules ranging from 1 to `r L`. Module probabilities on the 2-D embedding are scaled to range between 0 and 1. Each column on the heatmap represents a cell and each row represents a feature. Expression values for each feature are z-scored normalized across all cells after normalization. Red represents higher relative expression and blue represents lower relative expression. All cells are shown in the heatmap on the left. Only the top 250 cells with the lowest module probability and the 250 cells with the highest module probability are shown in the heatmap on the right. The column color bar displays the population assignment for each cell.
...
Relative expression profiles are shown for features that match the given list of markers. The parameter `exactMatch` is set to  ```r exactMatch```. If this parameter was set to ```FALSE```, then  additional features that contain the name of any marker may also be shown.

```{r plot_markers, fig.width = 15, fig.height = 100, message=FALSE, warning=FALSE, results = "asis"}
if (!is.null(features)) {
  ix <- retrieveSCEIndex(
    inSCE = sce,
    IDs = features,
...
print(celdaProbabilityMap(sce, useAssay = useAssay))
```

## Session Information
The file used for the SCE is ```r basename(sceName)```. The reducedDimName used was ```r basename(reducedDimName)```. The module file name is ```r basename(moduleFileName)```.
